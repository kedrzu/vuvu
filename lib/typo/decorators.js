...
export function AbstractType() {
    return genericTypo({}, true);
}
export function Type(idOrConfig) {
    if (typeof idOrConfig === 'string') {
        return genericTypo({ id: idOrConfig }, false);
    }
    return genericTypo(idOrConfig || {}, false);
}
function genericTypo(config, abstract) {
    return (constructor) => {
...
function getAllPropsMeta(target) {
    return getAllMetadata(propsSymbol, target);
}
...
export function getDescriptor(type) {
    ...
    let descriptor = typeof type === 'string' ? types[type] : type[typeSymbol];
